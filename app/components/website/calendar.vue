<script setup lang="ts">
	...
	const findUserByPhone = useDebounceFn(async (phone: string) => {
			...
			<table class="" :class="{}">
				<thead>
					<tr>
						<th class="bg-default">
							<div
								class="h-24 flex items-center justify-center border border-default"
							>
								HP
							</div>
						</th>
				...
				<template v-for="(row, rowIndex) in calendarStore.data" :key="rowIndex">
					<tr>
						<template v-for="(cell, cellIndex) in row" :key="cellIndex">
							<td v-if="cellIndex == 0" class="bg-default">
								<div class="py-4 px-1 border border-default">
									<div
										class="py-4 px-1 h-8 text-xs flex items-center max-w-24 text-center"
									>
										{{ cell.room.number }}
										<span
							...
							<td
								...
								"
							>
								<div v-if="cell.bookingInfo" class="p-4 border border-default">
									<div
